		...
		bool CityGenerator::connectToDeadEndInRadius(roadsegment_candidate& seg, float radius)
		{
			...
			for (auto iter = road_candidates_.begin(); iter != road_candidates_.end(); ++iter)
			{
				...
				//if(&candidate != &seg)
				{
					...
					{
						auto closest = iter;
						auto cross_iter = std::find_if(road_crossings_.begin(), road_crossings_.end(), [&seg](road_crossing& cross) {return cross.pt == seg.b; });
						...
						//int cross_road_count = 1 + std::distance(start, iter);
						//--iter;
